...
jQuery(document).ready(function ($) {
    ...
    function setDetailsFromAdCode(theAd) {
        ...
        $('#quads_settings\\[' + id + '\\]\\[g_data_ad_slot\\]').val(theAd.slotId);
        $('#quads_settings\\[' + id + '\\]\\[g_data_ad_client\\]').val(theAd.pubId);
        if ('normal' == theAd.type) {
            $('#quads_settings\\[' + id + '\\]\\[g_data_ad_slot\\]').val('normal');
            $('#quads_settings\\[' + id + '\\]\\[g_data_ad_width\\]').val(theAd.width);
            $('#quads_settings\\[' + id + '\\]\\[g_data_ad_height\\]').val(theAd.height);
        }
        if ('responsive' == theAd.type) {
            $('#quads_settings\\[' + id + '\\]\\[g_data_ad_slot\\]').val('responsive');
     ...
     * @param {type} content
     * @returns {quads-admin.min_L3.parseAdContent.theAd|Boolean}
     */
    function quadsParseAdSenseCode(content) {
        ...
        var asyncAd = theContent.find('ins');

        // Its a async adsense ad
        if (adByGoogle.length > 0) {
        ...
        if (adByGoogle.length === 0) {
            ...
            // Ad Slot ID
            theAd.slotId = adByGoogle.attr('google_ad_slot');

            if ('undefined' != typeof (adByGoogle.attr('google_ad_client'))) {
                // Ad Publisher ID
                theAd.pubId = adByGoogle.attr('google_ad_client').substr(3);
            }
