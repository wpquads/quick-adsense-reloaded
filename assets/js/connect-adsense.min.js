    ...
    $( document ).on( 'change', '#report_type,#report_period,#input_based,#report_view_type,#cust_fromdate,#cust_todate', function(){
        ...
        if($.trim(report_type) !='' & $.trim(report_period) !='' ) {
            ...
            setTimeout(function(){
                ...
                var data ={account_id:pub_id,report_type:report_type,report_period:report_period,input_based:input_based,cust_fromdate:cust_fromdate,cust_todate:cust_todate,report_view_type:report_view_type};
                ...
                $.ajax({
                    ...
                    error: function (request, status, error) {
                        $('#quads_reports_canvas').html('<h2> Please select other Date</h2>');
                        console.log('error');
                    },
                });
            }, 200);
...
function display_report(response){
    ...
    if(report_type == 'top_device_type') {
        ...
        var dynamic_tablet = '<table>' +
            '<tr><td>Desktop</td><td>Tablet</td><td>Mobile</td></tr>' +
            '<tr><td>'+desktop_total.toFixed(2)+'</td><td>'+tablet_total.toFixed(2)+'</td><td>'+mobile_total.toFixed(2)+'</td></tr>' +
            '</table>';
        ...
        for (index = 0; index < data_length; ++index) {
            ...
            if (dates_array.includes(New_date_formate)) {
                ...
                // current_data[1] = data[dates_index];
