...
function quads_admin_discount_btn() {
  if(! current_user_can( 'manage_options' )) {
        return ;
    }
  $screen = get_current_screen();
  $result = get_option( "quads_dismiss_discount_btn");
  $date_now = new DateTime();
  $date_exp = new DateTime("02/12/2020");
  if ($result != 'removed' && ($date_now > $date_exp) && 'toplevel_page_quads-settings' == $screen->base) {
  $date = date('d-m-y');
  if ($date == '01-12-20') {
      $msg = 'Only 24 Hours Remaining';
      $img = QUADS_PLUGIN_URL . 'assets/images/last-chance.png';
      $class_img = 'quads-offer-img-24';
      $class_text = 'quads-offer-text-bm-24';
  }else{
    $msg = 'Black Friday &amp; Cyber Monday';
    $img = QUADS_PLUGIN_URL . 'assets/images/bf-offer.png';
    $class_img = 'quads-offer-img';
    $class_text = 'quads-offer-text-bm';
  }
    $href = 'http://wpquads.com/festive-season/';
  ?>
  <div class="quads-wrapper-discount">
  <a class="quads-admin_discount_btn" target="_blank" href="<?php echo esc_url_raw($href) ?>">
    <img src="<?php echo esc_url($img) ?>" class="<?php echo esc_attr($class_img) ?>" >
    <span class="quads-offer-text-top" ><?php echo esc_html($msg) ?></span><br><span class="<?php echo esc_html($class_text) ?>">50% OFF on WP Quads</span></a>
    <span id='quads-offer-close'>x</span></div>

<?php } }
add_action('admin_footer', 'quads_admin_discount_btn');

function quads_dismiss_discount_btn(){
  if(! current_user_can( 'manage_options' )) {
        return ;
    }
    $result = update_option( "quads_dismiss_discount_btn", 'removed');
    if($result){
...
add_action('wp_ajax_quads_dismiss_discount_btn', 'quads_dismiss_discount_btn'); 