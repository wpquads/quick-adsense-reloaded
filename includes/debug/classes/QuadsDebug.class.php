...
if ( class_exists( "QuadsDebug" ) )
	return;

class QuadsDebug implements iQuadsDebug {
	...
	private function _get_interface( $interface_name ) {
		...
		switch ( $interface_name ) {
			...
			default:
				...
				if ( $interface && !in_array( 'iQuadsDebug', class_implements( $interface ) ) )
					$interface = null;
...
if ( !function_exists( 'quadsdebug' ) ) {
	/**
	 * Returns a QuadsDebug instance.
	 *
	 * @return QuadsDebug
	 */
	function quadsdebug() {
		return QuadsDebug::instance();
	}
}
